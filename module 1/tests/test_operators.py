...
def test_other(x):
    ...
    This test will check that def neg(x) will indeed return a negative value.
    """
    assert_close(operators.neg(x), -x)
...
def test_property(ls1, ls2):
    ...
    """
    assert_close(operators.sum(operators.addLists(ls1, ls2)), operators.sum(ls1) + operators.sum(ls2))
